def solution(array):
        ...
        if type_size[type] < 8:
            if len(block_table[current_block]) >= 8 : # 여기서 나머지가 0인 경우로 하니까 제일 처음 0인 경우도 0이니까 index 1부터 채워지네... 
                ...
                if type == "float":
                    ...
                    if len(block_table[current_block]) % 4 != 0:
                        padding = 4 - len(block_table[current_block]) % 4
                        block_table[current_block] += "." * padding
                    block_table[current_block] += "#" * type_size[type]
                
        else: # type의 크기가 8또는 16인 경우야.
            # 블락이 가득찼다면 그냥 8개씩 hash을 채워준다.
            if len(block_table[current_block]) >= 8:
                current_block += 1
                block_table[current_block] += "#" * 8
            ...
            # block이 가득차지 않았다면, 먼저 넣어줘야 하는데... 넣었을 경우 사이즈 초과가 발생해 그러면 패딩을 고려하여 block을 채우고 다음 블락에 넣어줘야 겠군
                ...
                if len(block_table[current_block]) + type_size[type] > 8: # 넣었을때 사이즈가 초과한다면
                    # 그렇다면 패딩은 얼만큼? 8- 나머지만큼
                    padding = 8 - len(block_table[current_block]) % 8
                    block_table[current_block] += "." * padding
                    # 패딩을 다 채워주었다면 위랑 같네
                    current_block += 1
                    block_table[current_block] += "#" * 8
    ...
    block_table = block_table[0:current_block+1]
    result = ','.join(block_table)
    return result
...
# 처음부터 ["", "", "", ...] 이런식으로 리스트가 구성하면 되는거네 굳이 2차원 리스트로 구현할 필요없이
